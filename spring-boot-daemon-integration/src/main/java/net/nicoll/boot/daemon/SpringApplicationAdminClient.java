...
class SpringApplicationAdminClient {
     ...
     * Returns {@code false} if the mbean is not yet deployed so this method
    ...
    private ObjectName toObjectName(final String name) {
        try {
    ...
    /**
     * Create a connector for an {@link javax.management.MBeanServer} exposed on
     * the current machine and the current port. Security should be disabled.
     *
     ...
     */
    static JMXConnector connect(final int port) throws IOException {
        String url = "service:jmx:rmi:///jndi/rmi://127.0.0.1:" + port
                + "/jmxrmi";
